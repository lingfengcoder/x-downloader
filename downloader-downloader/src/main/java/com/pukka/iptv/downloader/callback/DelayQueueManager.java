...
public class DelayQueueManager {
    ...
    private static volatile QueueInfo queueInfo;
    //内置延时队列
    ...
    private QueueInfo getQueueInfo() {
        if (queueInfo == null) {
            synchronized (this) {
                if (queueInfo == null) {
                    queueInfo = new QueueInfo().queue(resultConfig.getResultQueue())
                            .routeKey(resultConfig.getResultQueue())
                            .exchange(resultConfig.getResultExchange());
                }
            }
        }
        return queueInfo;
    ...
    private void put(DelayTask task) {
        ...
        if (delayQueue.size() > MAX_DELAY_QUEUE_SIZE) {
            MqSender.sendMsg(task.getData(), getQueueInfo());
        } else {
            delayQueue.put(task);
        }
    }
